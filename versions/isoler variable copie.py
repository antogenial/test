...
def get_param_by_name(param_name: str, params_list):
    for p in params_list:
        if p.name == param_name:
            return p

def  solve_system_for_params_expressions(params: list, syst_eq: list):
...
def scoring(eq, known_parameters, params_list):
    known_parameters = {get_param_by_name(key, params_list) for key in known_parameters}
...
def resolution(unknown, known_parameters, definitions, params_list):
    ...
    if unknown in definitions: # otherwise can't be resolve
        ...
        for i, eq in enumerate(definitions[unknown]):
            score = scoring(eq, known_parameters, params_list)
        ...
        while len(eqs) > 0:
            ...
            unknown_params = eq.free_symbols -  {get_param_by_name(key, params_list) for key in known_parameters}
            ...
            for p, def_list in available_definitions.items():
                for d in def_list:
                    if get_param_by_name(unknown, params_list) in d.free_symbols:
            ...
            for p in unknown_params:
                ...
                p_val = resolution(p.name, known_parameters, available_definitions, params_list) # recursive call
            ...
            if p_val is None: # unsolvable parameters -> unsovable unknow
                ...
                continue
            new_score = scoring(best_def, known_parameters, params_list)
            if new_score >= 1:
...
defs = solve_system_for_params_expressions(params, syst_eq)
...
known1 = {"d": 3, "r": 2}
unknown1 = "R"
R_val = resolution(unknown1, known1, defs, params) 
...
known2 = {"r": 1, "m": 10}
unknown2 = "R"
k_val = resolution(unknown2, known2, defs, params)
if k_val is not None:
    k_val = round(k_val, decimal, defs, params)
print("resut:", k_val)
print()
...
unknown3 = "d"
d_val = resolution(unknown3, known3, defs, params)
...
unknown4 = "h"
h_val = resolution(unknown4, known4, defs, params)
...
unknown5 = "R"
h_val = resolution(unknown5, known5, defs, params)
if h_val is not None:
    h_val = round(h_val, decimal)
print("resut:", h_val)
print()
